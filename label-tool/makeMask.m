...
% Last Modified by GUIDE v2.5 28-Feb-2020 20:35:09
...
if handles.slider_ZPos.Max < 2
    handles.slider_ZPos.Enable = 'off';
end
...
if handles.slider_TPos.Max < 2
    handles.slider_TPos.Enable = 'off';
end
...
if handles.slider_CPos.Max < 2
    handles.slider_CPos.Enable = 'off';
end
...
function refreshUI(h)
    if h.isMaskView
        ...
        imMat = double(h.data.rawData{h.curZPos,h.curChannel,h.curTPos});
        if h.isNormalize
            imMat = imMat/max(imMat(:));
            imMat = adapthisteq(imMat);
        end
        imMat = repmat(imMat/max(imMat(:)),[1,1,3]);
...
disp(strcat('successfully export and saved ',datestr(now)));
...
switch eventdata.Key
    ...
    case '5'
        ...
        handles.mixRatio = 1;
    case 'w'
        if handles.curTPos < handles.data.nSteps
            handles.curTPos = handles.curTPos + 1;
        end
    case 'q'
        if handles.curTPos > 1
            handles.curTPos = handles.curTPos - 1;
        end
    case 'c'
        handles.curLabel = max([0,handles.curLabel-1]);
        handles.slider_curLabel.Value = handles.curLabel;
    case 'v'
        handles.curLabel = min([handles.nLabel-1,handles.curLabel+1]);
        handles.slider_curLabel.Value = handles.curLabel;
end
guidata(hObject,handles);
refreshUI(handles);
...
curIm = double(handles.data.rawData{handles.curZPos,handles.curChannel,handles.curTPos});
...
%     handles.mask = segmentImage(im,str2double(answer{1}));
    handles.mask = double(segmentImage2(im));
end
guidata(hObject,handles);
refreshUI(handles);
...
% --- Executes on button press in btn_newLabelFromEdge.
function btn_newLabelFromEdge_Callback(hObject, eventdata, handles)
% hObject    handle to btn_newLabelFromEdge (see GCBO)
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)
handles.nLabel = handles.nLabel + 1;
handles.slider_curLabel.Min = 0;
...
handles.slider_curLabel.SliderStep = [1,1]/(handles.nLabel-1);
handles.labelColor = [handles.labelColor;0,0,0];
bw = handles.mask == handles.curLabel;
handles.curLabel = handles.nLabel - 1;
btn_done_Callback(handles.btn_done,eventdata,handles);

handles.mask(imdilate(edge(bw,'Sobel'),ones(3))) = handles.nLabel-1;
...
if eventdata.VerticalScrollCount > 0
    handles.bushSize = max([1,handles.bushSize-1]);
else
    handles.bushSize = handles.bushSize+1;
end
guidata(hObject,handles);
refreshUI(handles);